...
def test_regression_analysis_and_plot(sample_df: DataFrame,
                                      capsys: CaptureFixture[str]) -> None:
    """
    ...
    regression_analysis(sample_df, ax2)
    captured = capsys.readouterr()
    assert "Mean Absolute Error - Latitude:" in captured.out
    assert "Longitude" in captured.out

    points_plotted = (len(ax2.collections[1].get_offsets()) +
                      len(ax2.collections[2].get_offsets()))
    assert points_plotted == 2 * min((len(sample_df) / 5), 20)


def test_classification_analysis(sample_df: DataFrame,
                                 capsys: CaptureFixture[str]) -> None:
    """
    Tests the classification_analysis function in cse_163_final_project.py.
    """
    classification_analysis(sample_df)
    captured = capsys.readouterr()
